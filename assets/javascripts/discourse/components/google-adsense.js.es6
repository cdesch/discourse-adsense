...
if (Discourse.SiteSettings.adsense_publisher_code) {
  if (!mobileView && Discourse.SiteSettings.adsense_nth_slot_topic) {
    data["post-bottom"]["ad_code"] = Discourse.SiteSettings.adsense_nth_slot_topic;
    data["post-bottom"]["ad_width"] = parseInt(splitWidthInt("720"));
    data["post-bottom"]["ad_height"] = parseInt(splitHeightInt("90"));
  }
  if (mobileView && Discourse.SiteSettings.adsense_mobile_post_bottom_code) {
    data["post-bottom"]["ad_mobile_code"] = Discourse.SiteSettings.adsense_nth_slot_topic_mobile;
  }
}
